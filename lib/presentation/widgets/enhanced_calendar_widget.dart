...
class EnhancedCalendarWidget extends ConsumerWidget {
    ...
    try {
      ...
      for (final entry in tasksByDate.entries) {
        for (final task in entry.value) {
          ...
          final hasSpecificTime = task.dueDate != null && (task.dueDate!.hour != 0 || task.dueDate!.minute != 0);
                    ...
                    try {
                      if (details.visibleDates.isNotEmpty) {
                        ...
                        if (newDate.month != currentFocused.month ||
                            ...
                            (newDate.day != currentFocused.day && state.viewMode == CalendarViewMode.day)) {
  ...
  Color _getEnhancedTaskColor(TaskModel task, BuildContext context) {
    ...
    if (task.status == TaskStatus.completed) {
      ...
      return Color.fromARGB(255, (baseColor.r * 255.0 * 0.6).round() & 0xff, (baseColor.g * 255.0 * 0.6).round() & 0xff,
  ...
  Widget _buildTaskListItem(BuildContext context, WidgetRef ref, TaskModel task) {
    return Card(
      ...
      child: ListTile(
        ...
        leading: IconButton(
          icon: Icon(
            task.status == TaskStatus.completed ? PhosphorIcons.checkCircle() : PhosphorIcons.circle(),
