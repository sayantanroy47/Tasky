...
class _VoiceTaskCreationDialogM3State extends ConsumerState<VoiceTaskCreationDialogM3> with TickerProviderStateMixin {
      ...
      if (_speechEnabled) {
        ...
        try {
          ...
          if (mounted) {
            setState(() {
              _statusMessage =
                  _audioRecordingActive ? 'Audio recording only - speak now!' : 'Failed to start recording services';
            });
          }
        }
      } else {
    ...
    if (mounted) {
      setState(() {
        ...
        if (result.finalResult) {
          _statusMessage =
              _hasRecording ? 'Transcription complete! Continue or re-record' : 'No speech detected. Try again';
        } else {
  ...
  void _clearRecording() {
    setState(() {
      ...
      _audioRecordingActive = false;
      _statusMessage =
          _speechEnabled || _audioEnabled ? 'Tap to start dual stream recording' : 'Voice services not available';
    });
  }
