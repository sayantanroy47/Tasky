...
class RealDataExportService implements DataExportService {
    ...
    try {
      ...
      switch (format) {
        ...
        case ExportFormat.templatePackage:
          throw UnimplementedError('Export format ${format.name} is not yet implemented');
      }
  ...
  Future<void> shareData({
    ...
    await for (final progress in exportStream) {
      if (progress.progress == 1.0) {
        ...
        if (files.isNotEmpty) {
          final latestFile =
              files.whereType<File>().reduce((a, b) => a.statSync().modified.isAfter(b.statSync().modified) ? a : b);
  ...
  Future<void> _exportTasksToPDF(
    ...
    for (int pageIndex = 0; pageIndex < totalPages; pageIndex++) {
      ...
      final endIndex = (startIndex + tasksPerPage > tasks.length) ? tasks.length : startIndex + tasksPerPage;
