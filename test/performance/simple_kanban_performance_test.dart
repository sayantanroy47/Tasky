  ...
  group('Simple Kanban Performance Tests - Enterprise Scale', () {
    ...
    group('Large Dataset Processing Performance', () {
      ...
      test('Task processing with 1000+ tasks performs within benchmarks', () async {
        ...
        final avgDuration = tasksWithDuration.isNotEmpty 
            ? tasksWithDuration.fold<double>(0.0, (sum, t) => sum + (t.estimatedDuration ?? 0)) / tasksWithDuration.length
            : 0.0;
      ...
      test('Task processing with 2500+ tasks stress test', () async {
        ...
        final urgentTasks = tasks.where((t) => 
          t.title.toLowerCase().contains('urgent') || 
          t.priority == TaskPriority.urgent).toList();
      ...
      test('Task processing with 5000+ tasks extreme stress test', () async {
        ...
        for (final task in filteredTasks) {
          projectTaskCounts[task.projectId] = 
            (projectTaskCounts[task.projectId] ?? 0) + 1;
        }
    ...
    group('Search and Filter Performance', () {
      test('Complex search queries with large dataset', () async {
        ...
        for (int i = 0; i < searchQueries.length; i++) {
          ...
          expect(stopwatch.elapsedMilliseconds, lessThan(100), 
                 reason: 'Search query should complete within 100ms');
      ...
      test('Multi-criteria filtering performance', () async {
        ...
        print('Multi-criteria filtering (1500 tasks): ${stopwatch.elapsedMilliseconds}ms, ${filteredTasks.length} results');
      });
    });
    ...
    group('Sorting and Grouping Performance', () {
      test('Large dataset sorting performance', () async {
        ...
        final sortingTests = [
          ('by_due_date', (List<TaskModel> list) {
            list.sort((a, b) {
          ...
          ('by_priority', (List<TaskModel> list) {
            list.sort((a, b) => b.priority.index.compareTo(a.priority.index));
          }),
          ('by_created_date', (List<TaskModel> list) {
            list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
          }),
          ('by_title', (List<TaskModel> list) {
    ...
    group('Memory and Performance Validation', () {
      test('Memory usage simulation with large datasets', () async {
        ...
        final recentTasks = largeTasks.where((t) => 
          t.createdAt.isAfter(DateTime.now().subtract(const Duration(days: 30)))).toList();
        ...
        expect(memoryUsed, lessThan(500.0), 
               reason: '10k tasks should use less than 500MB');
        expect(memoryRecovered, greaterThan(memoryUsed * 0.5),
               reason: 'At least 50% of memory should be recoverable');
    ...
    tearDown(() {
      ...
      final categories = {
        ...
        '📊 Sorting/Grouping Performance': summary.entries.where((e) => e.key.contains('sort') || e.key.contains('group')),
